...
# [[character begin offset, character end offset], word index, word, lemma, pos tag]
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            group3OfSimilarRelationsForNounChild = ['tmod', 'prop_in', 'prop_at', 'prop_on']
            group4OfSimilarRelationsForNounChild = ['iobj', 'prop_to']
...
##############################################################################################################################
def collapse(parseResult):
    pr=parseResult
    deps_cc=parseResult['sentences'][0]['deps_cc']
    tokens=parseResult['sentences'][0]['tokens']
    try:
        for i in deps_cc:
            if(i[0]=='prep'):
                for j in deps_cc:
                    if(j[1]==i[2] and j[0]=='pobj'):
                        prep=tokens[j[1]]
                        pr['sentences'][0]['deps_cc'].remove(i)
                        pr['sentences'][0]['deps_cc'].remove(j)
                        pr['sentences'][0]['deps_cc'].append([u'prep_%s' % prep,i[1],j[2]])
                        break
            if('conj' in i[0]):
                tag=i[0]
                pr['sentences'][0]['deps_cc'].remove(i)
                pr['sentences'][0]['deps_cc'].append(['_'.join(tag.split(':')),i[1],i[2]])
...
def align(sentence1, sentence2):
    orig_len1 = len(sentence1.split(" "))
    orig_len2 = len(sentence2.split(" "))
    #import pdb; pdb.set_trace()
    sentence1ParseResult = collapse(parseText(sentence1))
    sentence2ParseResult = collapse(parseText(sentence2))
    try:
        sentence1Lemmatized = lemmatize(sentence1ParseResult)
        sentence2Lemmatized = lemmatize(sentence2ParseResult)
    except Exception as e:
        print 'Problem in lemmatize'
        raise e
    try:
        sentence1PosTagged = posTag(sentence1ParseResult)
        sentence2PosTagged = posTag(sentence2ParseResult)
    except Exception as e:
        print 'Problem in posTag'
        raise e


    if (len(sentence1Lemmatized) != orig_len1) or (len(sentence2Lemmatized) != orig_len2):
        """
        for i in xrange(len(sentence1Lemmatized)):
            if  i<orig_len1 and sentence1Lemmatized[i][2] <> sentence1.split()[i]:
                print sentence1Lemmatized[i][2] + '|' + sentence1.split()[i]
        for i in xrange(len(sentence2Lemmatized)):
            if i<orig_len2 and sentence2Lemmatized[i][2] <> sentence2.split()[i]:
                print sentence2Lemmatized[i][2] + '|' + sentence2.split()[i]

        """
        raise Exception('Inconsistent sentence length.')
